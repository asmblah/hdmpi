...
/**
 * Class VideoForwarderFactory.
 *
 * @author Dan Phillimore <dan@ovms.co>
 */
class VideoForwarderFactory implements WorkerSubFactoryInterface
{
    /**
     * @var object
     */
     ...
     * @inheritDoc
     */
    public function createForwarder(VideoForwarderSpec $forwarderSpec)
    {
        $videoSession = $this->sessionFactory->createMulticastSession(
            $forwarderSpec->getLocalHost(),
            $forwarderSpec->getLocalHost(),
            $forwarderSpec->getMulticastGroup(),
            $forwarderSpec->getVideoPort()
        );
        // Use a separate socket so that the source port can match.
        $videoSyncSession = $this->sessionFactory->createMulticastSession(
            $forwarderSpec->getLocalHost(),
            $forwarderSpec->getLocalHost(),
            $forwarderSpec->getMulticastGroup(),
            $forwarderSpec->getVideoSyncPort()
        );
